...
from pipeline.orchestration2 import RAGPipeline
...
def load_system(app):
    ...
    if profile == "public":
        ...
        synthesizer_llm = OpenAIClient(model_name=os.getenv("OPENAI_MODEL", "gpt-4o-mini"), temperature=0.2)
    elif profile == "gpu":
        llm = HFClient(model_name=os.getenv("HF_MODEL_GPU", "mistralai/Mistral-7B-Instruct-v0.2"), load_in_4bit=True, temperature=0.0)
        synthesizer_llm = HFClient(model_name=os.getenv("HF_MODEL_GPU", "mistralai/Mistral-7B-Instruct-v0.2"), load_in_4bit=True, temperature=0.2)
    ...
    evidence_relevance_judge = EvidenceRelevanceJudge(llm)
    query_expander = QueryExpander(llm)
    synthesizer = ResearchSynthesisEngine(synthesizer_llm, max_attempts=3)
